...
Analog design is governed by the Johnson Limit, a fundamental physical law stating that a transistor's breakdown voltage is inversely proportional to its maximum speed ($f_T$). Standard $130nm$ CMOS can survive $3V$, but it is far too slow for $112 Gbps$ ($56 GBaud$). By utilizing the IHP SG13G2 SiGe BiCMOS process, we leverage Heterojunction Bipolar Transistors (HBTs). The Germanium doping in the base of the HBT accelerates electron transit times, allowing the transistors to achieve staggering speeds ($f_T$ / $f_{max}$ > $300 GHz$) while still maintaining a safe breakdown voltage ($BV_{CEO}$) high enough to drive the photonics. [^4][^5]
